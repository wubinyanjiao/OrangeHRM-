      ...
      /*  var_dump($firsDocEmp);
        ...
        for($i=0;$i<$moth_count;$i++){
            ...
            if(null !== $firsDocEmp[$i]){
                ...
                //如果男士平均分配，有个平均值，比如3个，每个部门男士最少三人
                
                if(null == $secDocEmp[$i] && null !== $thirDocEmp[$i]){

                    //第一个部门和第三个部门有复合轮转的员工
                    //如果设置了男士平均分配，判断这个员工是否是男士
                    /*if($data['averge_man']['status']==1 ){


                        if($firsDocEmp[$i]['gender']==1){
                            //该部门员工中男士个数＝新加入的男士个数＋剩余的员工个数

                             $man_orang=$this->getRotaryMan($firsDocEmp,'gender',1);
                             $man_temp=$this->getRotaryMan($tmp_fir,'gender',1);

                             $total_man=count($man_orang)+count($man_temp)-1;
                             // var_dump($man_orang);exit;

                           
                        }
                        //首先判断如果这个员工轮转出去后加上新进入的员工数是否会低于平均值，如果高于平均值，完成该次轮转；如果不能满足，则这个部门选择第二个经验长的女士参与排班，
                    ...
                    if($data['averge_man']['status']==1 ){
                        if($firsDocEmp[$i]['gender']==1){
                            //该部门员工中男士个数＝新加入的男士个数＋剩余的员工个数
                             ...
                             $total_man_fir=count($man_orang_fir)+count($man_temp_fir)-2;

                             $man_orang_sec=$this->getRotaryManWomen($secDocEmp,'gender',1);
                             $man_temp_sec=$this->getRotaryManWomen($tmp_sec,'gender',1);
                             $total_man_sec=count($man_orang_sec)+count($man_temp_sec)-2;

                             $man_orang_thir=$this->getRotaryManWomen($thirDocEmp,'gender',1);
                             $man_temp_thir=$this->getRotaryManWomen($tmp_thir,'gender',1);
                             $total_man_thir=count($man_orang_thir)+count($man_temp_thir)-2;

                             //与平均值做对比，如果值小于平均值，这选择经验最长的女员工参与排班,并从原数组中删除该员工
                             if($total_man_fir<$data['averge_man']['count']){//第一个部门轮转
                                //获取部门中经验最多的女员工
                            ...
                            //第二个部门轮转
                            if($total_man_sec<$data['averge_man']['count']){//先判断男生是否平均，如果不平均
                            ...
                            //第三个部门轮转
                            if($total_man_thir<$data['averge_man']['count']){//先判断男生是否平均，如果不平均
                                ...
                                $tmp_thir[$i]=$thirDocEmp[$em_key_thir];
                                $tmp_thir[$i]['date_from']=$monarr[$i];
                                $tmp_thir[$i]['rotary_department']=$thirDocEmp[$i]['orange_department'];
                                unset($thirDocEmp[$em_key_thir]);
                            }else{
                                $tmp_thir[$i]=$thirDocEmp[$i];
                                $tmp_thir[$i]['date_from']=$monarr[$i];
                                $tmp_thir[$i]['rotary_department']=$thirDocEmp[$i]['orange_department'];
                                unset($thirDocEmp[$i]);
                            }
                        ...
                        if($data['rotary_limit']['status']==1){//门诊满几年轮转
                            if($data['min_age_rotary']['status']==1){//年龄满足至少多少不轮转到门诊
                                if($data['leader_no_rotary']['status']==1){//组长不参与轮转
                                    echo'<pre>';var_dump($firsDocEmp);exit;
