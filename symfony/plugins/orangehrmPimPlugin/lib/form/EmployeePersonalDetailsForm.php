...
class EmployeePersonalDetailsForm extends BaseForm {
    ...
    private function getMinzuList() {
        $list = array(0 => "-- " . __('Select') . " --");
        $getMinzuService = new MinzuService();
        $minzues = $getMinzuService->getMinzuList();
        foreach ($minzues as $minzu) {
            $list[$minzu->id] = $minzu->name;
        }
        return $list;
    }
    ...
    private function getPersonalInfoWidgets() {
        $widgets = array(
            ...
            'cmbMinzu' => new sfWidgetFormSelect(array('choices' => $this->getMinzuList())),
            ...
            'txtPolitics' => new sfWidgetFormInputText(),   //政治面貌
            'txtJoinedDate' => new ohrmWidgetDatePicker(array(), array('id' => 'personal_txtJoinedDate')),      //入职时间
        ...
        $widgets['cmbNation']->setDefault($this->employee->nation_code?$this->employee->nation_code:40);
        $widgets['cmbMinzu']->setDefault($this->employee->minzu_code);
        ...
        $widgets['txtPolitics']->setAttribute('value', $this->employee->emp_politics); //政治面貌
        $widgets['txtJoinedDate']->setAttribute('value', set_datepicker_date_format($this->employee->joined_date));  //入职时间
    ...
    private function getPersonalInfoValidators() {
        ...
        $validators = array(
            ...
            'cmbMinzu' => new sfValidatorChoice(array('required' => false, 'choices' => array_keys($this->getMinzuList()))),
            ...
            'txtPolitics' => new sfValidatorString(array('required' => false)),
            'txtJoinedDate' => new ohrmDateValidator(array('date_format' => $inputDatePattern, 'required' => false), array('invalid' => "Date format should be $inputDatePattern")),
        );
    ...
    public function getEmployee() {
        ...
        if ($this->personalInformationPermission->canUpdate()) {
            ...
            $minzu = $this->getValue('cmbMinzu');
            $employee->nation_code = ($nation != '0') ? $nation : null;
            $employee->minzu_code = ($minzu != '0') ? $minzu : null;
            ...
            $employee->joined_date = $this->getValue('txtJoinedDate');

            $employee->emp_politics = $this->getValue('txtPolitics');
        }
