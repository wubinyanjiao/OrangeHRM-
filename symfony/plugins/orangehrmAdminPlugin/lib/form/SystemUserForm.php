...
class SystemUserForm extends BaseForm {
    ...
    public function save() {
        ...
        if (empty($userId)) {
            ...
            $user->setEmpNumber($user->_data['emp_number']);   //没有值
        }
        //根据员工名称找id 
        
       
       // $this->getSystemUserService()->getEmployeeList($empData['empName'],$empId=0);
       // var_dump($this->getSystemUserService()->getEmployeeList(3));
       //  echo '<pre>';var_dump($user->_data['emp_number']);die;
       //  end 
        $user->setUserRoleId($this->getValue('userType'));
        $user->setEmpNumber($empData['empId']);   //没有值
    ...
    protected function getFormLabels() {
        $required = '<em> *</em>';
        $labels = array(
            'userType' => __('用户角色') . $required,
            'employeeName' => __('员工姓名') . $required,
            'userName' => __('用户名') . $required,
            'password' => __('密码') . '<em class="passwordRequired"> *</em>',
            'confirmPassword' => __('确认密码') . '<em class="passwordRequired"> *</em>',
            'status' => __('状态') . $required,
            'chkChangePassword' => __('修改密码'),
        );
