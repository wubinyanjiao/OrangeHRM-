 ...
 */
class ApplyOvertimeForm extends sfForm {
...
//var_dump($values);die;
    ...
    public function save() {
       ...
       //echo '<pre>';var_dump($_POST);echo '<hr>'; var_dump($posts);die;
        $applyOvertime = new ApplyOvertime();
        
        //$applyOvertime->current_day = '12-12-12';
        $applyOvertime->emp_number = $posts['txtEmployee']['empId'];
        $applyOvertime->creat_time = date('Y-m-d');
        $applyOvertime->current_day = $posts['txtCurrentDate'];
         
       
      // var_dump($_POST['txtFromDate_1']);die;
         $applyOvertime->stat_time = $_POST['assignleave']['txtFromDate_1'];
         $applyOvertime->end_time = $_POST['assignleave']['txtToDate_1'];
        $applyOvertime->content = $posts['txtComment'];
        if(!empty($posts['recurring'])){
            $applyOvertime->is_holiday = 1;
        }
        $applyOvertime->status = 2;    //2正常 3取消
   
        $applyOvertime->save();

        $OvertimeId = $applyOvertime->getId();
        return $OvertimeId;
    }
    ...
    public function getJavaScripts() {
        ...
        $javaScripts[] = plugin_web_path('orangehrmLeavePlugin', 'js/applyOvertimeSuccess.js?v=2');
    ...
    protected function getFormWidgets() {
        ...
        $widgets = array(
            ...
            'txtCurrentDate' => new ohrmWidgetDatePicker(array(), array('id' => 'assignleave_txtCurrentDate')),
            ...
            'txtFromDate_1' =>new ohrmWidgetTimeDropDown(array(), array('id' => 'assignleave_txtFromDate_1')),
            'txtToDate_1' =>new ohrmWidgetTimeDropDown(array(), array('id' => 'assignleave_txtToDate_1')),


           'recurring' => new sfWidgetFormInputCheckbox(array()),
    ...
    protected function getFormValidators() {
        ...
        $validators = array(
            ...
            'txtCurrentDate' => new ohrmDateValidator(array('date_format' => $inputDatePattern, 'required' => true),
            ...
            // 'txtToDate' => new ohrmDateValidator(array('date_format' => $inputDatePattern, 'required' => true),
               //     array('invalid' => 'Date format should be ' . $inputDatePattern)),
            'txtFromDate_1' => new sfValidatorTime(array(
                    'required' => true,
                    'time_format' => "/(?P<hour>2[0-3]|[01][0-9]):(?P<minute>[0-5][0-9])/",
                    'time_output' => 'H')),
            'txtToDate_1' => new sfValidatorTime(array(
                    'required' => true,
                    'time_format' => "/(?P<hour>2[0-3]|[01][0-9]):(?P<minute>[0-5][0-9])/",
                    'time_output' => 'H')),

            'recurring' => new sfValidatorString(array('required' => false)),    
    ...
    protected function getFormLabels() {
        ...
        $labels = array(
           ...
           'txtCurrentDate' => __('加班日期'). $requiredMarker,
          // 'txtCurrentDate' => __('加班日期 '),
            'txtFromDate_1' => __('From Date') . $requiredMarker,
            'txtToDate_1' => __('To Date') . $requiredMarker,
