 ...
 */
class applyOvertimeAction extends baseLeaveAction {
    ...
    public function execute($request) {
       ...
       // $this->form = $this->getAssignLeaveForm($this->leaveTypes);   
        $this->form = $this->getAssignLeaveForm();             
        $this->overlapLeave = 0;
        ...
        if ($request->isMethod('post')) {     
            ...
            if ($this->form->isValid()) {
                try {
                    $OvertimeId = $this->form->save();

                   
                    if ($OvertimeId) {
                        $this->getUser()->setFlash('success', __('Successfully Assigned'));
                        //$this->redirect('leave/addEmployee');
                    } else {
     ...
     */
    protected function getAssignLeaveForm($leaveTypes=null) {

        $leaveFormOptions = array('leaveTypes' => $leaveTypes);
        $form = new ApplyOvertimeForm(array(), $leaveFormOptions, true);
