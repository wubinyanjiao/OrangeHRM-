...
class EmployeeDao extends BaseDao {
     ...
     * @returns integer  
     * @throws DaoException
     *     根据attach_id 数组获取多条数据
     */
    public function getsomeEmployeeAttachments($empNumber, $entriesToDelete = null) {
        try {
            
            $q = Doctrine_Query::create()->from('EmployeeAttachment')
                                         ->where('emp_number = ?', $empNumber);
