...
class viewPhotographAction extends basePimAction {
    ...
    public function execute($request) {
        //echo '<pre>';var_dump($_SERVER['CONTEXT_DOCUMENT_ROOT']);die;
    ...
    private function saveEmployeePicture($empNumber, $file) {
        ...
        //图片只路径存入数据库 修改原代码图片存入数据库 
        $new_file = uplaodeimg_files($file,'head_pic');
        if($new_file){
            $old_url = $empPicture->imgurl;
            $empPicture->imgurl = $new_file;
            if(!empty($old_url)){
                //$url = $_SERVER['CONTEXT_DOCUMENT_ROOT'].$old_url;
                $url = substr_url($old_url);
                unlink ($url);
            }
