...
class EmployeeJobDetailsForm extends BaseForm {
    ...
    public function configure() {
        ...
        $this->setWidgets(array(
            ...
            'eeo_cat_describe' => new sfWidgetFormTextarea(),
            'emp_cost_center' => new sfWidgetFormTextarea(),
            ...
            'probation_date' => new ohrmWidgetDatePicker(array(), array('id' => 'job_probation_date')),
            'formal_date' => new ohrmWidgetDatePicker(array(), array('id' => 'job_formal_date')),
            //'joined_date' => new ohrmWidgetDatePicker(array(), array('id' => 'job_joined_date')),
        ...
        $this->setDefault('eeo_cat_describe', $employee->eeo_cat_describe);
        $this->setDefault('emp_cost_center', $employee->emp_cost_center);
        ...
        $this->setDefault('probation_date', set_datepicker_date_format($employee->probation_date));
        $this->setDefault('formal_date', set_datepicker_date_format($employee->formal_date));
        ...
        $this->setValidators(array(
            ...
            'eeo_cat_describe' => new sfValidatorString(array('required' => false)),
            'emp_cost_center' => new sfValidatorString(array('required' => false)),
            ...
            'joined_date' => new ohrmDateValidator(
                    array('date_format' => $inputDatePattern, 'required' => false),
                    array('invalid' => 'Date format should be ' . $inputDatePattern)),
            'probation_date' => new ohrmDateValidator(
                    array('date_format' => $inputDatePattern, 'required' => false),
                    array('invalid' => 'Date format should be ' . $inputDatePattern)),
            'formal_date' => new ohrmDateValidator(
                    array('date_format' => $inputDatePattern, 'required' => false),
                    array('invalid' => 'Date format should be ' . $inputDatePattern)),
    ...
    public function getEmployee() {
        ...
        $eeo_cat_describe = $this->getValue('eeo_cat_describe');
        if ($eeo_cat_describe == '') {
            $employee->eeo_cat_describe = null;
        } else {
            $employee->eeo_cat_describe = $eeo_cat_describe;
        }
        ...
        $emp_cost_center = $this->getValue('emp_cost_center');
        if ($emp_cost_center == '') {
            $employee->emp_cost_center = null;
        } else {
            $employee->emp_cost_center = $emp_cost_center;
        }
...
//        asort($subUnitList);
    ...
    public function updateAttachment() {
        ...
        if ($update == self::CONTRACT_DELETE) {   //删除
            //删除之间查询是否有文件
            $sq = Doctrine_Query :: create()->from('EmployeeAttachment a')
                            ->where('emp_number = ?', $empNumber)
                            ->andWhere('screen = ?', "contract");
            $attachmentList = $sq->fetchOne();
            $file_url = substr_url($attachmentList->eattach_attachment_url);                 
        ...
        } else if ($update == self::CONTRACT_UPLOAD || !empty($file)) {  //替换
            // find existing
            ...
            if ($result->count() == 1) {
                ...
                $file_url = '';
                $file_url = substr_url($empAttachment->eattach_attachment_url);
            }
            ...
            $new_url = uplaodeimg_files($file,'fujian');
            if($new_url){
                $empAttachment->eattach_attachment_url = $new_url;
            }else{
